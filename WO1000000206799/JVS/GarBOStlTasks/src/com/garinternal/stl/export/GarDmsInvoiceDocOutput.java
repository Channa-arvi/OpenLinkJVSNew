...
/*
File Name:                      GarDmsInvoiceDocOutput.java
...
Description:
Output script for Invoice Documents
...
import com.garinternal.common.enums.GAR_STLDOC_INFO_ENUM;
...
public class GarDmsInvoiceDocOutput extends GarDmsBasicOutput {
     ...
     * Constructor
     *
     * @param logger {@link GarLogger}
     */
    protected GarDmsInvoiceDocOutput(GarLogger logger) {
        super(logger);
        this.logger = logger;
    }
    ...
    public Table getOutputDocFileData(int queryId, String filePrefix) throws OException {
        ...
        sqlSelect.append("\n SELECT     DISTINCT sol.document_num, sol.output_doc_file, si.value AS gar_doc_num, td.total_docs"); //
        ...
        String sqlJoins = "\n FROM       stldoc_output_log sol" //
            ...
            + "\n LEFT  JOIN stldoc_info si ON si.document_num = sol.document_num" //
            + "\n        AND si.type_id = " + GAR_STLDOC_INFO_ENUM.D_GAR_DOCUMENT_NUMBER.toInt(); //
    ...
    public Table outputDocs(Table tblDocs, Table tblUserParams) throws OException {
        ...
        tblResults.addCols("S(status) S(gar_doc_num) S(export_path) S(error_message)");

        this.addFolderConfig(tblDocs);

        // col number for tblDocs
        int colNumDmsPath      = tblDocs.getColNum("output_doc_file");
        int colNumGarDocNum    = tblDocs.getColNum("gar_doc_num");
        int colNumCountry      = tblDocs.getColNum("country");
        ...
        int colNumResGarDocNum  = tblResults.getColNum("gar_doc_num");
        ...
        for (int rowNum = 1; rowNum <= totalRows; rowNum++) {
            ...
            try {
                ...
                String garDocNum    = tblDocs.getString(colNumGarDocNum, rowNum);
                int    country      = tblDocs.getInt(colNumCountry, rowNum);
                ...
                folderName = this.enhanceFolderName(folderName, folderPrefix, country);

                tblResults.setString(colNumResGarDocNum, rowNum, garDocNum);
                ...
                String renamedFilePath = this.changeOutputFileName(fileName, exportPathCurrent, fileExtension, garDocNum);
        ...
        tblResults.sortCol("gar_doc_num");
     ...
     */
    private String enhanceFolderName(String folderName, String folderPrefix, int countryId) throws OException {
        String finalName = "";

        if (GAR_COUNTRY_ENUM.SINGAPORE.toInt() == countryId || GAR_COUNTRY_ENUM.INDONESIA.toInt() == countryId) {
            finalName = folderPrefix + "_INV";
        } else {
            finalName = folderPrefix + "_INV_" + folderName;
        }
