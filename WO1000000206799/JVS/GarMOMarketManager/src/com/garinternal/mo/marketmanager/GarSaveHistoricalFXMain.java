...
public class GarSaveHistoricalFXMain extends GarBasicScript {
    ...
    public void execute(Table argt, Table returnt) throws OException {
        ...
        try {
            ...
            tblHistoricalFX.addCols("I(fx_rate_date) I(currency_id) I(reference_currency_id) F(fx_rate_bid) F(fx_rate_mid) "
                + "F(fx_rate_offer) I(data_set_type) I(currency_convention)");
    ...
    private Table getFXIdxList() throws OException {
        ...
        String sql = "\n SELECT * " //
            ...
            + "\n   AND db_status = " + IDX_DB_STATUS_ENUM.IDX_DB_STATUS_VALIDATED.toInt()//
            + "\n ORDER BY index_id ASC"; //

        String errorMsg =
            "Unable to retrieve index group = " + idxGroupId + " and db status = " + IDX_DB_STATUS_ENUM.IDX_DB_STATUS_VALIDATED.toInt();
